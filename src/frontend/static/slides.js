...
let slideshowType = 'family'; // Will be overridden  
...
function initializeFromServer() {
    if (window.slideshowConfig) {
        slideshowType = window.slideshowConfig.slideshowType;
...
function get_fetch_script_args() {
    ...
    if (mode === "random") {
        script_args = `?embeddings_file=${encodeURIComponent(embeddings_file)}`;
        ...
        if (swiper && swiper.slides && swiper.slides.length > 0) {
            ...
            if (currentSlide && currentSlide.dataset.filepath) {
                currentFilepath = currentSlide.dataset.filepath;
            }
        }
        script_args = `?current_image=${encodeURIComponent(currentFilepath)}&embeddings_file=${encodeURIComponent(embeddings_file)}`;
    } else {
        throw new Error("Invalid mode specified. Use 'random' or 'sequential'.");
    }
    return script_args;
}
...
// MUST FIX: the backend logic for retrieve_next_image is not considering the search
// file list.
async function fetchRandomImage() {
    let spinnerTimeout = setTimeout(() => showSpinner(), 500); // Show spinner after 0.5s
    try {
        let script_args = get_fetch_script_args();
        let response, data;

        if (searchResults && searchResults.length > 0) {
            // Use POST with searchResults and current slide
            let currentFilepath = "**start**";
            if (swiper && swiper.slides && swiper.slides.length > 0) {
                const currentSlide = swiper.slides[swiper.activeIndex];
                if (currentSlide && currentSlide.dataset.filepath) {
            ...
            response = await fetch('retrieve_next_image', {
                ...
                body: `files=${encodeURIComponent(JSON.stringify(searchResults))}` +
                    `&current_image=${encodeURIComponent(currentFilepath)}` +
                    `&embeddings_file=${encodeURIComponent(embeddings_file)}`
            });
        } else {
            response = await fetch(`retrieve_next_image${script_args}`);
        }
...
async function addNewSlide() {
    ...
    const path = data.path; // Full path to the image
    ...
    img.addEventListener('dragstart', function (e) {
        e.dataTransfer.setData('DownloadURL', `image/jpeg:${data.filename || 'image.jpg'}:${data.url}`);
...
function updateOverlay() {
    ...
    if (!slide) return;
    document.getElementById('descriptionText').innerHTML = slide.dataset.description || "";
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (storedHighWaterMark !== null) highWaterMark = parseInt(storedHighWaterMark, 10);

    const storedCurrentDelay = localStorage.getItem('currentDelay');
    if (storedCurrentDelay !== null) currentDelay = parseInt(storedCurrentDelay, 10);
    ...
    // Check if the slideshow has gotten stuck in iOs.
    // setInterval(() => {
    //     if (swiper && swiper.autoplay && swiper.autoplay.running) {
    //         // Check if the active slide hasn't changed for a long time (e.g., 2x currentDelay)
    //         // If so, try to restart autoplay
    //         // Optionally, update your icons here as well
    //     }
    // }, 5000);
    ...
    swiperContainer.addEventListener('touchend', handleTouchEnd, { passive: false });
    ...
    doSearchBtn.addEventListener('click', async function () {
        ...
        // concatenate the slideshow path with the embeddings file
        const embeddings_file = joinPath(PICS[slideshowType][0], 'embeddings.npz')
    ...
    clearSearchBtn.addEventListener('click', function () {
        ...
        if (slideShowRunning)
            resumeSlideshow(); // Resume slideshow if it was running
    });
    ...
    delete_current_file_btn.addEventListener('click', async () => { await deleteCurrentFile(); });
...
async function searchWithImage(file, first_slide) {
    ...
    try {
        ...
        const response = await fetch('https://servo.4crabs.org/search_with_image/', {
            method: 'POST',
            body: formData
        });
        const result = await response.json();
...
function updateSearchCheckmarks() {
    ...
    let checkOverlay = searchIcon?.parentElement?.querySelector('.checkmark-overlay');
...
async function deleteCurrentFile() {
    ...
    try {
        ...
        const response = await fetch(`random_pictures.pl?delete=${encodeURIComponent(filepath)}`);
        // check status
        if (!response.ok) {
...
function getCurrentFilepath() {
    // Adjust this to however you get the current image filename
...
async function clearSearchAndResetCarousel() {
    ...
    if (typeof textSearchPanel !== "undefined") {
        textSearchPanel.style.opacity = 0;
        setTimeout(() => { textSearchPanel.style.display = 'none'; }, 200);
    }
...
function setCheckmarkOnIcon(iconElement, show) {
    ...
    let checkOverlay = iconElement?.parentElement?.querySelector('.checkmark-overlay');
    if (checkOverlay) checkOverlay.remove();
