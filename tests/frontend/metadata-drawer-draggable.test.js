...
describe("Metadata Drawer Draggable Functionality", () => {
  ...
  describe("Mouse dragging", () => {
    ...
    it("should not drag when clicking on non-draggable elements", () => {
      ...
      // Get initial position (not used in this test, but kept for reference)
      drawer.style.left;
      drawer.style.top;
  ...
  describe("Smooth dragging behavior", () => {
    it("should calculate position based on delta from start for smooth tracking", () => {
      ...
      expect(positionsRecorded[0]).toEqual({
        left: initialRect.left + 10,
        top: initialRect.top + 10,
        deltaX: 10,
        deltaY: 10,
      });
      expect(positionsRecorded[1]).toEqual({
        left: initialRect.left + 25,
        top: initialRect.top + 15,
        deltaX: 25,
        deltaY: 15,
      });
      expect(positionsRecorded[2]).toEqual({
        left: initialRect.left + 50,
        top: initialRect.top + 40,
        deltaX: 50,
        deltaY: 40,
      });
    });
