...
### Viewing InvokeAI Video Metadata

InvokeAI 7 records a generated video's parameters inside the `.mp4`
file itself, the same way it records an image's inside the `.png`.
PhotoMapAI reads them, so a generated video's drawer shows its prompt,
model, seed and LoRAs just as an image's does, followed by the
video-specific parameters: the generation mode, the frame count and
frame rate, the first and last frame images, and the source clip and
frame range for an extended video. Where the record names a single-file
model assembled from parts — a standalone text encoder, a low-noise
expert — those are listed too, since the main model name alone does not
say what ran.

The still-frame panel above it continues to report what the *file*
contains — duration, resolution, codec and container — so the two
panels together cover both the request and the result.
